const SheetsDB = {
  ...
  async _apiAppend(type, record) {
    ...
    const doAppend = () => fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${CONFIG.SURVEY_SPREADSHEET_ID}/values/${encodeURIComponent(sheet)}!A1:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`,
      {
        method: 'POST',
    ...
    if (res.status === 400) {
      // La pestaña probablemente no existe — crearla y reintentar
      await this._createSheet(CONFIG.SURVEY_SPREADSHEET_ID, sheet, token);
      res = await doAppend();
    }

    if (!res.ok) {
      const errBody = await res.json().catch(() => ({}));
      throw new Error(`Sheets API ${res.status}: ${errBody.error?.message || 'error desconocido'}`);
    }
    return record;
  },
